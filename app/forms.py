...
'''def check_for_len(value):
    if len(value)<6:
        raise forms.ValidationError('len is less than 6')'''
class studentform(forms.Form):
    ...
    botcatchers=forms.CharField(max_length=100,widget=forms.HiddenInput,required=False)
    ...
    def clean_botcatchers(self):
        bot=self.cleaned_data['botcatchers']
        if len(bot)>0:
